...
def loss(old_p,old_v,p_nn,v_nn,obv,actions,adv,alpha):    
    ...
    act_one_hot=tf.one_hot(actions,number_of_actions)#TODO:set number of actions
    clip_epsilon=tf.math.multiply(alpha,base_clip_epsilon)#TODO:set base_clip_epsilon
    ...
    total_loss=tf.reduce_mean(clip_loss+value_loss_coefficient*value_loss+entropy_loss_coefficient*entropy_loss)#TODO set coefficient parameters
...
def gradients(optimizer):  
    ...
    def grads(old_p,old_v,p_nn,v_nn,obv,actions,adv,alpha):
        ...
        clipped_grads,_=tf.clip_by_global_norm(initial_grads,maximum_gradient)#TODO:set max gradient here
